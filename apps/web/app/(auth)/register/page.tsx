...
export default function RegisterPage() {
  ...
  const strength = password.length === 0 ? null
    : password.length < 6  ? { label: 'Too short', color: '#EF4444', w: '20%' }
    : password.length < 8  ? { label: 'Weak',      color: '#F59E0B', w: '45%' }
    : password.length < 12 ? { label: 'Good',      color: '#60A5FA', w: '72%' }
    : { label: 'Strong', color: '#00FF88', w: '100%' }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#07080C', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative' }}>
        ...
        <div style={{ position: 'fixed', top: '-200px', left: '-200px', width: '600px', height: '600px', borderRadius: '50%', background: 'rgba(0,255,136,0.04)', filter: 'blur(130px)', pointerEvents: 'none', zIndex: 0 }} />
        <div style={{ position: 'fixed', bottom: '-150px', right: '-150px', width: '500px', height: '500px', borderRadius: '50%', background: 'rgba(167,139,250,0.025)', filter: 'blur(110px)', pointerEvents: 'none', zIndex: 0 }} />
        ...
        <div style={{
          ...
          <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: '#00FF88', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '8px' }}>// get started</p>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '34px', fontWeight: 700, color: '#F0F2F7', letterSpacing: '-0.5px', marginBottom: '6px', lineHeight: 1.1 }}>Create account</h1>
          <p style={{ fontSize: '14px', color: '#6B7280', marginBottom: '32px' }}>
            Already have one?{' '}
            <Link href="/login" style={{ color: '#00FF88', textDecoration: 'none', fontWeight: 500 }}>
              Sign in →
            </Link>
          </p>
          ...
          <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: focused === 'name' ? '#00FF88' : '#6B7280', marginBottom: '7px', transition: 'color 0.2s' }}>
                Full name
              </label>
              <input value={name} onChange={e => setName(e.target.value)}
                onFocus={() => setFocused('name')} onBlur={() => setFocused(null)}
                placeholder="Aryan Sawhney" required
                style={inp('name')} />
            </div>
              ...
              {strength && (
                <div style={{ marginTop: '8px' }}>
                  <div style={{ height: '3px', background: 'rgba(255,255,255,0.05)', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: strength.w, background: strength.color, borderRadius: '2px', transition: 'all 0.3s ease' }} />
                  </div>
                  <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: strength.color, marginTop: '5px' }}>{strength.label}</p>
                </div>
              )}
            </div>
